...
## Session 13: MQ2 烟雾趋势图 + 数据卡 UI 精修

**Date**: 2026-05-18
**Task**: MQ2 烟雾趋势图 + 数据卡 UI 精修
**Branch**: `dev`
...
实装 MQ2 烟雾趋势图（前端 panel 视图的 `.data-card`）：解析平铺 UTF-8 遥测帧、推 90s 滚动窗口、自绘平滑 SVG 曲线带三条参考线、`mq2_alarm=1` 时整卡变红。在用户反馈后做了一轮 UI 精修：缩小数据卡顶部 hero 区、提高图表实际可视化高度、LED 卡与电机卡等高、移除右上角"X 点 / 实时时间"元信息。

## 改动一览

| 区域 | 改动 |
|------|------|
| 新模块 | `web/js/telemetry-panel.js` — `parseTelemetryLine` + `createTelemetryPanel({card,status,value,chart})`，缓冲跨包 WS 文本、按 `\n` 切行、忽略空/畸形/无 `mq2` 的行、90s 滚动窗口、SVG viewBox 640×320 自绘 |
| 集成 | `web/js/main.js` 在 `onFrame` 里把 frame 转交给 telemetry-panel（终端展示保留） |
| 视图 | `web/index.html` 新增 MQ2 卡片 DOM；后续移除 `.mq2-meta`（90 点 / 实时时间） |
| 样式 | `web/css/style.css` 新增 MQ2 卡片整套样式；hero 字号 48→32、padding/gap 收紧；`.control-cards` `align-items: stretch` 让 LED 与电机等高；`.led-body` 加 `flex: 1`；图表 min-height 320→360；响应式断点同步 |
| 规范 | `quality-guidelines.md` 新增 telemetry-panel 的 ownership / signature / validation matrix / 手工测试用例；`.data-card` 从 reserved-placeholder 表移除；view-switching 段文案更新 |
| 文档 | `docs/architecture.md` 模块图 + telemetry 节点；deferred 列表移除 "Telemetry chart"；`directory-structure.md` 加 telemetry-panel.js |
| README | mermaid 节点标签简化（HTML label → 纯文本 + 命名边），独立 commit `dcf1069` |

## 验证

- `npm run lint` / `npm run format:check` / `npm test` 全过
- 手工：本地 `127.0.0.1:8080` 面板视图确认布局正确，LED 与电机卡等高、图表占满下方空间
- 部署：用户报告远端 `wss://websocket.vaple.cc` 仍是旧 UI；根因是 sirv 静态文件没发 `Cache-Control: no-store`，浏览器/Cloudflare 缓存了旧资源。已建议先硬刷新/清 CF 缓存，长期修复方案（给 sirv 加 `setHeaders` 强制 no-store）已提出但**未实施**，等用户确认。

## 后续可能项

- (开放) server/src/static.js 加 `Cache-Control: no-store`，避免下次 UI 改动遇到同样的 CDN/浏览器缓存问题
- 未来若接入设备真实 ppm 标定，把 hero 数值的语义从"趋势指数"切换到 ppm 时需要同步改 UI 文案与参考线
...
## Session 14: Mobile responsive adaptation + WS reconnect race fix

**Date**: 2026-05-19
**Task**: Mobile responsive adaptation + WS reconnect race fix
**Branch**: `dev`
...
| Area | Description |
|------|-------------|
| Mobile layout | Responsive CSS in `web/css/style.css` for narrow viewports — config panel, terminal, AI view, panel view all stack/reflow cleanly on phone-sized screens |
| WS race fix | `web/js/ws-client.js` now captures `activeSocket` per `connect()` call; every `open`/`message`/`close`/`error` handler short-circuits when `socket !== activeSocket`. Stale `open` self-closes with code 1000 / reason `'stale connection'` |
| Unit tests | New `test/ws-client.test.js` (Node test runner) covering rapid disconnect/reconnect race and out-of-order open ordering; `npm run test:unit` wired into `npm test` |
| Lint | `eslint.config.js` extended so the new test file lints cleanly |
| Spec | `quality-guidelines.md` gains validation-matrix rows + a "WS client active-socket guard" design decision + required test list |
| Docs | `README.md` / `README.zh-CN.md` mention stale-socket guard and the new `test:unit` script |

**Updated Files**:
- `web/css/style.css`
- `web/js/ws-client.js`
- `test/ws-client.test.js`
- `package.json`
- `eslint.config.js`
- `.trellis/spec/frontend/quality-guidelines.md`
- `README.md`
...
**Verification**:
- `npm run lint`, `npm run test:unit`, `npm test` all green
- Manual mobile-viewport check across config panel / terminal / AI / panel views
